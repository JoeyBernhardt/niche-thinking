...
## Example data (taken from KBS time series)
# '2017_A_10-CH28_4_RI_03-TM-10-3-R2'
egdat<-data.frame(time=c(17.9284722,9.9145833,13.9541667,9.0125000,12.9284722,16.9291667,0.9722222,11.0013889,0.0000000,8.0131944,11.9333333,18.9062500,4.0229167,3.0333333,7.0062500,5.9847222,20.9340278,19.9395833,4.8868056,14.9965278,16.0180556,1.8909722),
				  log.fluor=c(5.112714,4.948780,5.096441,4.588206,5.092295,5.119763,1.449947,5.110046,0.781819,4.255234,4.998089,5.055782,2.663951,2.328480,3.899183,3.413726,4.966042,4.953977,2.900249,5.226657,5.121840,1.808143))

par(mfrow=c(1,1))
plot(exp(log.fluor)~time,data=egdat)

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #

# Set up a Monod model:

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #

# Set up parameters and initial conditions
parameters<-c(r=1.1,K=500000,Y=1,m=0.1,R0=exp(14.8))
state<-c(N=8100)
...
parameters<-c(r = 1.5, alpha = 0.005)
state<-c(N=100)
...
# Set up Monod model for MLE2 fit:

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #


# Wrap Monod model ODE in a function that's easier to use with bbmle/mle2
monod.ode<-function(time.vals,N0,r,m,R0,K,verbose=F,maxtime){
  
  parameters<-c(r=r,m=m,R0=R0,K=K)
  state<-c(N=N0)
...
parameters<-c(r=0.85,K=100,Y=1,m=0.1,R0=181)
state<-c(N=exp(0.781819))

maxtime<-max(egdat$time)

N0.guess<-min(egdat$log.fluor)
R0.guess<-max(egdat$log.fluor)

### Run fit (slow)

# With my first set of starting guesses, the fit doesn't quite converge
fit.monod.1<-mle2(log.fluor~dnorm(mean=monod.ode(time,exp(N0),r,exp(m),exp(R0),exp(K),maxtime=maxtime,verbose = T),sd=exp(s)),start=list(N0=N0.guess,r=0.85,m=log(0.1),R0=R0.guess,K=log(100),s=-0.1),data=egdat)
